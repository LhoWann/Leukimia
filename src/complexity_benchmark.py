...
from lightning_model import ConvNeXtV2Classifier, CoAtNetClassifier
...
def main():
    ...
    ap.add_argument('--out', default='results_comparison/complexity.md')
    args = ap.parse_args()
    ...
    models = [
        ('ConvNeXtV2-Tiny (Ours, no MHA)',
         ConvNeXtV2Classifier(num_classes=2, pretrained=False, use_mha=False)),
        ('CoAtNet-0 (baseline)',
         CoAtNetClassifier(num_classes=2, pretrained=False)),
    ]
    ...
    L = ['# Kompleksitas Model & Latensi', '',
         f'Input 224×224, FP32, device **{dev_name}**. Latensi = warmup 10 + rata-rata '
